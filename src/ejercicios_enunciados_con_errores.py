...
def menu_hotel():
    ...
    print("2. Reservar habitación (introduciendo su nombre)")
...
def reservar_habitaciones():
    try:
        habitacion = int(input("🛏️ ¿Qué habitación quieres reservar?: "))
        if habitacion in reservas:
            print("❌ Lo siento, esa habitación ya está reservada.")
        elif habitacion not in habitaciones:
            print("⚠️ Esa habitación no existe.")
        else:
            print("✅ Esa habitación está libre.")
            nombre = input("👤 Nombre del cliente: ")
            reservas[habitacion] = nombre
            print(f"📦 Habitación {habitacion} reservada a nombre de {nombre}.")
    except ValueError:
        print("⚠️ Ese número no es válido.")

def cancelar_reserva():
    try:
        habitacion = int(input("❎ ¿Qué habitación deseas cancelar?: "))
        if habitacion in reservas:
            del reservas[habitacion]
            print(f"✅ Reserva de la habitación {habitacion} cancelada.")
        else:
            print("⚠️ No hay ninguna reserva para esa habitación.")
    except ValueError:
...
def ver_reservas():
    if reservas:
        ...
        for habitacion, nombre in reservas.items():
            print(f"🛏️ Habitación {habitacion} - Cliente: {nombre}")
    else:
...
# Diccionario para guardar las reservas,
reservas = {}

# Lista de habitaciones válidas,
habitaciones = list(range(101, 111))

# Bonus: Carga inicial aleatoria de algunas reservas,
def carga_aleatoria():
    nombres = ["Ana", "Luis", "Carlos", "Lucía", "Pedro"]
    habitaciones_ocupadas = random.sample(habitaciones, k=random.randint(1, 5))
    for habitacion in habitaciones_ocupadas:
        reservas[habitacion] = random.choice(nombres)

# Opción 1: Ver habitaciones disponibles con mapa visual,
def ver_habitaciones_disponibles():
    print("\n📋 Mapa de habitaciones:")
    for habitacion in habitaciones:
        if habitacion in reservas:
            print(f"Habitación {habitacion}: ❌ Ocupada por {reservas[habitacion]}")
        else:
            print(f"Habitación {habitacion}: ✅ Disponible")
    print()

# Opción 2: Reservar habitación,
def reservar_habitacion():
    try:
        num = int(input("Ingrese el número de habitación a reservar (101-110): "))
        if num not in habitaciones:
            raise KeyError("Habitación inválida.")
        if num in reservas:
            print("⚠️ Esa habitación ya está ocupada.")
        else:
            nombre = input("Ingrese su nombre: ")
            reservas[num] = nombre
            print(f"✅ Habitación {num} reservada exitosamente por {nombre}.\n")
...
def cancelar_reserva():
    try:
        num = int(input("Ingrese el número de habitación a cancelar (101-110): "))
        if num not in habitaciones:
            raise KeyError("Habitación inválida.")
        if num in reservas:
            del reservas[num]
...
# Opción 4: Ver todas las reservas,
...
# Menú principal,
def menu():
    carga_aleatoria()  # Carga inicial aleatoria
    while True:
        print("=== Simulador de Reservas de Hotel ===")
        print("1. Ver habitaciones disponibles")
