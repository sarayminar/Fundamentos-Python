...
# def vote_film():
#     name = input("¿A qué película quieres votar?: ").strip()
#     try:
#         films[name]["votes"] += 1
#         print(f"Voto registrado. '{name}' ahora tiene {films[name]['votes']} votos.")
#     except KeyError:
...
# if not parrafo:
#     raise ValueError("El párrafo no puede estar vacío.")
...
#     except ValueError:
#         print("Precio o stock no válido.")
...
reservas = {}
def menu_hotel():
    opcion = print("Habitaciones del 101 al 110, ¿qué deseas hacer?")
...
def reservar_habitaciones():
    try:
        habitacion = int(input("¿Qué habitación quieres reservar?"))
        if habitacion in reservas:
        ...
        print("Ese número no es una habitación")
